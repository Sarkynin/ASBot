...
class LearnPlugin
  ...
  def execute(m, msgtype, pokemon, move)
    ...
    pokerow = @db[:pokemon].filter(:identifier => BotUtils.condense_name(pokemon))
    if pokerow.nil?
      BotUtils.msgtype_reply(m, msgtype, "Pokémon not found.")
      return
    end
    pokeid = pokerow[:id]
    pokename = @db[:pokemon_species_names].filter(:local_language_id => 9, :pokemon_species_id => pokerow[:species_id])
    ...
    movename  = @db[:move_names].filter(:local_language_id => 9, :move_id => moveid)

    movefound = !@db[:pokemon_moves].filter(:pokemon_id => pokeid, :move_id => moveid).nil?
